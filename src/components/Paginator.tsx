...
import DropDown from "./DropPageNumber";
...
export default function Paginator(props: IPaginatorProps) {
  ...
  const endIndex = Math.min(props.pageNumber * props.pageSize, props.totalRecords);

  const visiblePages = totalPages > 5 ? 5 : totalPages;
  let startPage = Math.max(1, currentPage - Math.floor(visiblePages / 2));
  startPage = startPage > totalPages - visiblePages + 1 ? totalPages - visiblePages + 1 : startPage;
