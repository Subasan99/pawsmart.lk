...
export default function Index() {
  ...
  const [selectedSelectedSpecializations, setSelectedSpecializationst] = useState<
  any | undefined
>(null);
  const [specializations, setAllSpecializations,filterspecializations,setAllFilterSpecializations] = useSpecializationStore(
    (state: any) => [state.specializations, state.setAllSpecializations,state.filterspecializations, state.setAllFilterSpecializations]
  );
  ...
  async function fetchData() {
    const data = await getSpecializationData(1, 10,selectedSelectedSpecializations);
    ...
    {isFiltersOpen && (
        <div className="flex gap-4 items-center">
          ...
          <div className="flex-1">
            <Select onValueChange={(value) => setSelectedSpecializationst(value)}>
              ...
              <SelectContent>
                {specializations.length > 0 ? (
                  specializations.map((item: any) => (
                    <SelectItem key={item.id} value={String(item.specializationName)}>
      ...
      <DataTable columns={columns} data={filterspecializations} />
    </div>
  );
}
